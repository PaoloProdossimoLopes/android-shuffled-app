...
//        throw Error()
    ...
    override fun postDeck(deck: CreateDeckModel, onComplete: CreateDeckClientCompletionBlock) {
        ...
        inmemoryDecks.add(DeckListResponse.Deck(id + 1, deck.title, deck.description, deck.imageUri.toString(), deck.isFavorited, deck.cards.map { DeckListResponse.Card(it.id, it.question, it.answer) }))
...
//                throw Error()
    ...
    override suspend fun findBy(id: Int): DeckResponse? {
        ...
        inmemoryDeck?.let {
            return DeckResponse(DeckResponse.Deck(it.id, it.title, it.description, it.thumbnailUrl, it.isFavorited, it.cards.map { DeckResponse.Card(it.id, it.question, it.answer) }))
        }
    ...
    override suspend fun updateDeck(deck: Deck): Boolean {
        ...
        inmemoryDecks[deckIndex].cards = deck.cards.map { DeckListResponse.Card(it.id!!, it.question, it.awnser) }
